...
As you saw in class, you will need to use CS 211’s \progname{dev} command
to get access to the correct version of the C compiler we’re
using\marginnote{The GNU C Compiler, version 8.3.1}. Additionally,
...
these hidden files as well.} files in your home
directory. The \progname{dev} command must be defined in a file
called \filename{.tcshrc}, and Emacs looks for its configuration in a
file called \filename{.emacs}.
...
restarts. You can log out and back in, or the command \cmdline{exec
...
a .tgz \marginnote{ The .tgz file extension is used for
“\href{https://en.wikipedia.org/wiki/Gzip}{gzipped}
\href{https://en.wikipedia.org/wiki/Tar_(computing)}{tarball},” which is
like the Linux equivalent of a .ZIP file. } file that you need to
downloaded onto your remote shell. Linux provides a command called
\progname{curl} that we will use to download a
``tarball''\marginnote{``tar'' stands for ``tape archive''; it’s a kind
of archive file, like ZIP, but most commonly used on Unix systems.}, and
...
\begin{CmdLine}
  \prompt curl \metavar{url} | tar zxvk
\end{CmdLine}

\marginnote[-\baselineskip]{The letters after \texttt{tar} give the
details of what to do: \texttt z specifies the kind of compression that
the archive uses, \texttt x means to extract (rather than create or
list), \texttt v increases the verbosity (without it, \progname{tar} is
silent), and \texttt k prevents \progname{tar} from overwriting existing
files.}
