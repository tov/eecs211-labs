...
\begin{itemize}
  ...
  \item Use Northwestern’s virtual private network (VPN). When you
    ...
    To setup the NU VPN, follow \hyperlink{\GlobalProtectUrl}{these
    instructions}.

  \item Use SSH to ``proxy jump'' through a different host than the one
    we ultimately want. You can setup SSH to do this automatically for
    you, but it’s a bit harder than using the VPN. Instructions for
    setting up a proxy appear in the per-system sections below.
...
you don’t have to use the VPN, try \hyperlink{\PuTTYhelpUrl}{these
instructions}.
...
\hyperlink{https://en.wikipedia.org/wiki/Key_authentication}{cryptographic
