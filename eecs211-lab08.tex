...
Let's say that you have a \functionname{print} function for a base class.
Normally, if that function were not void, and you were to write the following code (assuming we have the shapes from above, where both the \varname{Shape} and each of the derived classes have a \functionname{print} function defined):
\begin{Code}
    Circle* circ;
    Shape* shp = circ;
    shp->print();
...
Since \varname{shp} is using its \functionname{print} function, which is nota virtual function, even though \varname{shp} points to a \varname{Circle}, we are still using the base, \varname{Shape} class's \functionname{print} function

Now, if our \varname{Shape} had its \functionname{print} function as a virtual function, we would instead be using the derived, \varname{Circle} print function.
