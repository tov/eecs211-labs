...
// This is an abstract type in C. Whoever #includes this file will know
// that some `struct cons_pair` exists, but they won't know its size or
// its fields. So they can't create one! But they can still access one
// if given a pointer to it.
...
// Filters list `lst` by predicate function `keep`, returning a
// new list_t that retains only those elements of `lst` satisfying
// `keep`. Borrows `lst`, and the caller takes ownership of the result.
