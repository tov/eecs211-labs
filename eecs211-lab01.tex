...
directory to \filename{lab1} using the \progname{cd} command.  Now list
its contents using \progname{ls}, and notice that there is a
\filename{CMakeLists.txt} file, and a \filename{main.cpp} file. The
\filename{CMakeLists.txt} file is a CMake configuration file which you
won't have to worry about too much, and the \filename{main.cpp} file is
...
the lab1 directory.} to set up the build environment.  This should spit
out some commands into the terminal, and if everything is successful,
your CMake project is set up!

Now, you can build your main program using the command \progname{make}.
...
this case, run \cmdline{make main} to build a program called
\filename{main}.  In your build directory, if you list its contents, you
will see a few more CMake files, but you should also now see a file
called \filename{main}, which is your executable you can now run:
...
Navigate back up to your \filename{lab1} directory using cd (\cmdline{cd
..}).  Now open up the \filename{main.cpp} file using Emacs, and edit it
so it now says ``Aloha 211 student!'' instead of ``Hello 211 student!.''
Make sure to save and exit Emacs.\marginnote{Remember \keycombo{C-x C-s}
to save and \keycombo{C-x C-c} to exit.}

Go back to our build directory using cd.  Try running \cmdline{./main}
...
\progname{make}.  So now, run \cmdline{make main} once again, and try
\cmdline{./main}. Notice how you now have the correct output!  Each time
