...
be found here: \LabHostnamesLink}
...
\noindent where \metavar{eecs-id} is your EECS username (probably your
NetID) and \metavar{eecs-host} is replaced by one of the EECS hostnames
from the list of student lab hostnames (such as
\hostname{batman.eecs.northwestern.edu}). When prompted, type in your
...
\begin{CmdLine*}
  \C emacs src/lab2.c\\
\end{CmdLine*}
Notice that there is already some skeletons of functions and some code in \functionref{main}{} here.

\subsection{Iteration}

First, find the function called
\functionref{sum\_numbers}{}.\marginnote{Notice that
\functionref{sum\_numbers}{} is going to return an {\codeKeywordstyle
...
called \filename{lab2}. To run it, the command is:
...
\filename{lab2.c} you will notice that we created an ^apple^ structure
...
Make and run \progname{lab2}, and see if your new animal shows up the way that
...
Make and run \progname{lab2}, and see if everything still works!  
...
Make and run \progname{lab2} and see if this feature is working!
